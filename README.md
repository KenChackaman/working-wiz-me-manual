...
画面は **楽天らしい赤（#BF0000系）を基調にした社内向けの簡易トンマナ**です。

- 画面上部の **マーク（`assets/mark.svg`）** は **オリジナル**です（公式の楽天「R」ロゴの複製ではありません）。
- **「楽天 / Rakuten」表記**はタイポグラフィによる **簡易表現**です。正式なロゴ・厳密な配色・フォントは **社内の Rakuten Brand & ReX Guidelines** に従って差し替えてください。

### 公式ロゴを載せたい場合（任意）

社内手順で利用が許可されたロゴファイルを `assets/` に置き、`index.html` の該当箇所（`brand-lockup`）を画像表示に差し替えるのが安全です（ファイル名は社内ルールに合わせてください）。
